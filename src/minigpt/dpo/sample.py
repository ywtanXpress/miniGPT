...
log = get_logger("minigpt.dpo.sample")
...
def dpo_sample(
    ...
    if ckpt_path is None:
        ckpt_path = find_latest_checkpoint(paths["out_dir"])
    log.info("Loading DPO checkpoint: %s", ckpt_path)
    ...
    if use_amp and device.type == "cuda":
        with torch.autocast(device_type="cuda", dtype=amp_dtype):
            out = model.generate(x, max_new_tokens=max_new_tokens, temperature=temperature, top_k=top_k, eos_token_id=eos_id)
    else:
        out = model.generate(x, max_new_tokens=max_new_tokens, temperature=temperature, top_k=top_k, eos_token_id=eos_id)
